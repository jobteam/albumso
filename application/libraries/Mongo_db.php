...
Class Mongo_db {
    ...
    private function connect() {
        ...
        try {
            ...
            $this->connect = new MongoClient("mongodb://" . $this->hostname, array('username' => $this->username, 'password' => $this->password, 'db' => $this->database));
    ...
    /**
     * --------------------------------------------------------------------------------
     * // tungns develop : tim kiem theo tu khoa cua SP
     * db.getCollection('demo').find( { 'tags':{ $all: ["sons", "tungns1"] } }  )
     * --------------------------------------------------------------------------------
     *
     ...
     */
    public function findProductTags($collection = "", $option = array()) {
        if (empty($collection)) {
            show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
        }
        try {
            var_dump($option);
            return $this->db->{$collection}->find(array('PO100' => 568));
        } catch (MongoCursorException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
    ...
    public function findOne($collection = "", $option = array()) {
        if (empty($collection)) {
            show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
        }
        try {
            //var_dump($option);die;
            return $this->db->{$collection}->find(array('_id' => 271));
        } catch (MongoCursorException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
                show_error("MongoDB query failed: {$e->getMessage()}", 500);
            } else {
    ...
    public function findCollection($collection = "") {
        ...
        try {
            return $this->db->{$collection}->find(array('PO100' => 568, '_id' => 118));
        } catch (MongoCursorException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
                show_error("MongoDB query failed: {$e->getMessage()}", 500);
            } else {
    ...
    public function pullMongo($collection, $options = array(), $wheres = array()) {

        if (empty($collection)) {
            show_error("No Mongo collection selected for update", 500);
        }
        try {
            $this->db->{$collection}->update($wheres, $options);
            $this->_clear();
            return (TRUE);
        } catch (MongoCursorException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
    ...
    public function updateCollectionSet($collection = "", $options = array(), $wheres = array()) {
        if (empty($collection)) {
            show_error("No Mongo collection selected for update", 500);
        }
        try {
     ...
     */
    public function updateCollection($collection = "", $options = array(), $wheres = array()) {
        if (empty($collection)) {
            show_error("No Mongo collection selected for update", 500);
        }
        try {
     ...
     */
    public function deleteMongo($collection = "", $wheres = array()) {
        if (empty($collection)) {
            show_error("No Mongo collection selected to delete from", 500);
        }
        try {
            $this->db->{$collection}->remove($wheres);
            $this->_clear();
            return (TRUE);
        } catch (MongoCursorException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
    ...
    public function getAll($collection = "", $limit, $skip) {
        if (empty($collection)) {
            show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
        }
        try {
    ...
    public function getNextSequence($name = '') {
        try {
            $collection = 'Q250';
            $ret = $this->db->{$collection}->findAndModify(
                    array('_id' => $name), array('$inc' => array('sequence_value' => 1)), array(), array('new' => true)
            );
